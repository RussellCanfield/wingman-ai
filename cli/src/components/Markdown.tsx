...
import {highlightSyntax, defaultSyntaxTheme, type SyntaxTheme} from './SyntaxHighlighter';
...
const defaultTheme = {
  ...
  link: (text: string, href: string): string => `${chalk.blue.underline(text)} ${chalk.dim.gray(`→ ${href}`)}`,
  blockquote: (text: string): string => chalk.yellow.italic(`▐ ${text}`),
  hr: (): string => chalk.dim('━'.repeat(60))
};

const renderInlineTokens = (tokens: any[], theme: typeof defaultTheme): string => {
  if (!tokens) return '';
  ...
  return tokens.map((token: any) => {
    switch (token.type) {
      case 'text':
        return token.text;
      case 'strong':
        return theme.strong(renderInlineTokens(token.tokens || [], theme) || token.text);
      case 'em':
        return theme.em(renderInlineTokens(token.tokens || [], theme) || token.text);
      case 'codespan':
        return theme.codespan(token.text);
      case 'link':
        return theme.link(renderInlineTokens(token.tokens || [], theme) || token.text, token.href);
      case 'br':
        return '\n';
      default:
...
const renderMarkdownToString = (markdown: string, theme: typeof defaultTheme, syntaxTheme: SyntaxTheme): string => {
    ...
    switch (token.type) {
      ...
      case 'list':
        const listItems = token.items.map((item: any) => {
          ...
          if (item.tokens && item.tokens.length > 0) {
            ...
            itemText = item.tokens.map((itemToken: any) => {
              if (itemToken.type === 'text') {
                return renderInlineTokens(itemToken.tokens || [], theme) || itemToken.text;
      ...
      case 'code':
        ...
        const formattedLines = lines.map((line: string, index: number) => {
          const lineNum = chalk.dim.cyan((index + 1).toString().padStart(lineNumWidth, ' '));
          const separator = chalk.dim(' │ ');
          const codeLine = line.replace(/\t/g, '    '); // Convert tabs to spaces
          const highlightedCode = highlightSyntax(codeLine, token.lang || '', syntaxTheme);
          return chalk.dim('│ ') + lineNum + separator + highlightedCode;
        });
        
        // Add bottom border that extends only under the line numbers
        const bottomBorder = chalk.dim('└' + '─'.repeat(lineNumWidth + 2) + '┘');
        
        return `\n${chalk.dim('┌─ ')}${langLabel}\n${formattedLines.join('\n')}\n${bottomBorder}\n\n`;
      
      case 'blockquote':
        const quoteText = token.tokens.map((t: any) => renderToken(t)).join('').trim();
        const quoteLines = quoteText.split('\n').filter((line: string) => line.trim());
        const quotedLines = quoteLines.map((line: string) => '  ' + theme.blockquote(line.trim()));
        return '\n' + quotedLines.join('\n') + '\n\n';
...
const Markdown: React.FC<MarkdownProps> = ({children, theme, syntaxTheme}) => {
  ...
  try {
    ...
    const renderedMarkdown = renderMarkdownToString(children, mergedTheme, mergedSyntaxTheme);
    ...
    return (
      <Box flexDirection="column">
        <Text color="red">Error parsing markdown: {error instanceof Error ? error.message : 'Unknown error'}</Text>
