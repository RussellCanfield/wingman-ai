...
const messageClassTypeCache = new Map<
	MessageUnion | MessageChunkUnion,
	string
>();
...
export function filterMessages(
	...
	if (Array.isArray(messagesOrOptions)) {
		const filtered: BaseMessage[] = _filterMessages(messagesOrOptions, options);
...
function ensureToolUseResultIntegrity(messages: BaseMessage[]): BaseMessage[] {
	...
	for (let i = 0; i < workingMessages.length; i++) {
		...
		if (toolUseIds.length > 0) {
			...
			if (nextIndex >= workingMessages.length) {
				...
				const filteredContent = (msg.content as any[]).filter(
			...
			const matchingToolResults: Record<string, boolean> = {};
...
async function _lastMaxTokens(
	...
	let messagesCopy = messages.map((message) => {
		...
		return _switchTypeToMessage(msgType, fields, isBaseMessageChunk(message));
	});
...
function _chunkToMsg(chunk: BaseMessageChunk): BaseMessage {
	...
	const fields = Object.fromEntries(
		Object.entries(chunk).filter(
			([k]) =>
				!["type", "tool_call_chunks"].includes(k) && !k.startsWith("lc_"),
		),
	) as BaseMessageFields;
