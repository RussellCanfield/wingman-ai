...
export class CodeSuggestionProvider implements InlineCompletionItemProvider {
	...
	async provideInlineCompletionItems(
		...
		const [topContent, bottom, positionFromBottom] = this.getPromptContent(document, position, inline);
		if (timeout) {
		...
		return new Promise<InlineCompletionItem[]>((res) => {
			timeout = setTimeout(() => {
				this.bouncedRequest(topContent, abort.signal, bottom, inline, position, positionFromBottom)
					.then(items => {
						res(items);
					});
			}, 300);
	...
	private getPromptContent(document: TextDocument, position: Position, inline: boolean): [string, string, Position] {
		let topContent = '';
		if (!inline) {
		...
		if (!inline) {
			...
			positionFromBottom = new Position(lastPosition.line - after.line, 0);
		}

		return [topContent, bottom, positionFromBottom];
	}

	private stripTopAndBottom(code: string, topPos: Position, fromBottom: Position) {
		let linePost = 0;
		let lineCount = 0;
	...
	async bouncedRequest(prompt: string, signal: AbortSignal, doc: string, inline: boolean, topPos: Position, fromBottom: Position): Promise<InlineCompletionItem[]> {
		try {
			const codeResponse = await aiService.codeComplete(prompt, signal, [], doc);
			let cleanedCode = '';
			if (!inline) {
				cleanedCode = this.stripTopAndBottom(codeResponse, topPos, fromBottom);
			}
			else {
