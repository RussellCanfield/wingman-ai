...
A new experimental feature has been added called "Validate". Validate is available and appears after a Composer run complets successfully. It allows you to configure a command to be run via a shell (command line on windows, and bash on macOS) with up to a 60s timeout.
