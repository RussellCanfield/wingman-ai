...
export function ToolEventPanel({
	toolEvents,
	variant = "panel",
}: ToolEventPanelProps) {
	const sorted = (toolEvents as RichToolEvent[]).slice().sort((a, b) => {
...
function ToolEventCard({ event }: { event: RichToolEvent }) {
...
function resolveActorLabel(event: RichToolEvent): string | null {
...
function summarizeInvokedAgents(events: RichToolEvent[]): Array<{
	label: string;
	total: number;
	running: number;
}> {
...
function AlertTriangleIcon({ className }: IconProps) {
	return baseIconPath({
		...
		children: (
			<>
				<path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.72-3L13.71 3.86a2 2 0 0 0-3.42 0z" />
				<path d="M12 9v4" />
				<path d="M12 17h.01" />
			</>
		),
