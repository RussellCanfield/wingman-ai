...
- Dedicated app root at `apps/windows`
- Native backend scaffold at `apps/windows/src-tauri`
...
```bash
# from repo root
bun run --cwd apps/windows test
bun run --cwd apps/windows build:web
bun run --cwd apps/windows dev:web
```
...
bun run --cwd apps/windows tauri:dev
bun run --cwd apps/windows tauri:build
```
