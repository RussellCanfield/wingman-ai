...
export class LlamaChatSession {
    ...
    private _conversationHistoryToLoad:
        | readonly ConversationInteraction[]
        | null = null;
    ...
    public constructor({
        ...
        if (promptWrapper === "auto") {
            ...
            if (chatWrapper != null) this._promptWrapper = new chatWrapper();
            else this._promptWrapper = new GeneralChatPromptWrapper();
    ...
    public async init() {
        await withLock(this, "init", async () => {
            ...
            if (this._printLLamaSystemInfo) console.log("Llama system info");
        ...
        return await withLock(this, "prompt", async () => {
            ...
            if (
                ...
                const { text, stopString, stopStringSuffix } =
                    generateContextTextFromConversationHistory(
                        this._promptWrapper,
                        this._conversationHistoryToLoad,
                        {
    ...
    private async _evalTokens(
        ...
        for await (const chunk of evaluationIterator) {
            ...
            if (shouldReturn) {
                ...
                const skippedChunksText =
                    skippedChunksQueue.length > 0
                        ? this._ctx.decode(Uint32Array.from(skippedChunksQueue))
                        : "";

                let [queuedTextBeforeStopString] =
                    skippedChunksText.split(stopString);

                queuedTextBeforeStopString =
                    queuedTextBeforeStopString.trimEnd();

                if (queuedTextBeforeStopString.length > 0) {
                    const beforeStopStringTokens: Token[] = Array.from(
                        this._ctx.encode(queuedTextBeforeStopString)
                    );
