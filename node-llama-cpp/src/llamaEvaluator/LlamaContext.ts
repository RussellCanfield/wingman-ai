...
import { LLAMAContext } from "../utils/getBin.js";
...
export class LlamaContext {
	...
	public constructor({
		...
		const LLAMAContext = model._context!;
	...
	public decode(tokens: Uint32Array | Token[]): string {
		...
		if (tokens instanceof Uint32Array) return this._ctx.decode(tokens);
	...
	public async *evaluate(
		...
		if (this._prependTokens.length > 0) {
			const tokenArray: Token[] = this._prependTokens.concat(
				Array.from(tokens)
			);
		...
		while (true) {
			...
			const nextToken: Token = await this._ctx.eval(
				evalTokens,
				removeNullFields({
					temperature,
					topK,
					...
					repeatPenaltyTokens:
						repeatPenalty?.punishTokens instanceof Function
